...
abstract class CoreDate {
  ...
  static String dateController({required DateTime date, required Locale locale}) => _Date.dateController(date: date, locale: locale);
...
class _Date {
  ...
  static final DateFormat _formatTR = DateFormat('dd.MM.yyyy HH:mm:ss.SSS', 'tr_TR');
  static final DateFormat _formatUK = DateFormat('dd/MM/yyyy HH:mm:ss.SSS', 'en_GB');
  static final DateFormat _formatUS = DateFormat('MM/dd/yyyy hh:mm:ss.SSS a', 'en_US');
  ...
  static String dateController({required DateTime date, required Locale locale}) {
    final now = DateTime.now();
    if (date.isAfter(now)) {
      throw ArgumentError('The date cannot be in the future.',);
    }
    else {
      if (locale.languageCode == 'tr') {
        ...
        if (locale.countryCode == 'US') {
          ...
          throw ArgumentError('Unsupported English locale: ${locale.countryCode}');
        }
      }
    }
